#!/usr/bin/env python
#
# Modified from https://github.com/mantl/terraform.py
#
...
@parses('linode_linode')
@calculate_mantl_vars
def linode_host(resource, tfvars=None):
    raw_attrs = resource['primary']['attributes']
    name = raw_attrs['name']
    groups = []
    ...
    # attrs specific to Mantl
    attrs.update({
        'consul_dc': _clean_dc(attrs['region']),
        'role': 'none',
        'ansible_python_interpreter': 'python'
    })

    # add groups based on attrs
    groups.append('linode_group=' + attrs['group'])
