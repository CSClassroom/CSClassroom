...
namespace CSC.CSClassroom.Service.UnitTests.Questions.QuestionGeneration
{
	...
	public class QuestionModelFactory_UnitTests
	{
		...
		/// <summary>
		/// Ensures that the resulting question model excludes the
		/// question IsPrivate property, since the permissions of a
		/// question must always be constant.
		/// </summary>
		[Fact]
		public void GetQuestionModel_ExcludesIsPrivateProperty()
		{
			var expectedResult = new List<JavaClass>();
			var builder = GetMockJavaModelBuilder(expectedResult);
			var questionModelFactory = GetQuestionModelFactory(builder.Object);
			...
			(
				...
				(
					It.Is<Func<PropertyInfo, bool>>
					(
						expr => expr(typeof(Question).GetProperty("IsPrivate")) == true
					)
				),
				Times.Once
			);
